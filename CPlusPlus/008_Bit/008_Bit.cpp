...
int main()
{
    ...
    // 만약 1 001(-1) + 0 010(+2)를 하면 0이 나와야한다.
    // 하지만 1 011(-3)가 나오게 되었다.
    // 우리가 원하는 연산을 하기위해 편의성을 부여한 것이 음수의 비트를 반전시키는 방법이다.(1의 보수)
    // -1을 표현한다면 아래와 같다.

    // 1 001 -> 1 110 (숫자bit 반전)

    // 이제 우리는 1110을 -1로 생각하기로 했다. 계산해보자

    // 1110(-1) + 0010(+2) = 0000(실제론 10000이 되는데 5자리이므로 앞자리는 떼주어야 한다.)

    // 앞자리를 떼어야 하는 이유는 4자리수 연산을 가정하여 맨 앞자리를 부호bit로 정했기 때문이다.
    // 만약 부호비트 이상으로 자리수가 올라가면 당연히 해당 논리의 연산이 불가능하다.
  
    // 위 결과값를 보면 우리가 알고있는 -1 + 2 = 1이 나와야하는데 0000(0)이 나온다.
    // 여기서 모든 연산에서 1의 오차가 생겨버리는 문제가 생긴 것이다.
    // 그 이유는 우리가 알고있는 0은 한 개인데 위 연산규칙 으로 0이 두 개가 생겨버린 것이다. ( 0000(+0), 1111(-0) )
    // 이 문제는 간단하게 해결하였다.
    // 결과에 +1을 해주어 양수를 1씩 밀어버린 것이다. (2의 보수)
    // 그럼 수식은 아래와 같다.
    // 
